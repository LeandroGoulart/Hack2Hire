...
(function() {
    ...
    const analysisPanorama = document.getElementById('analysisPanorama');
    ...
    function updateSelectedCount() {
        ...
        updateAnalyzeAction(count);
    }

    function updateAnalyzeAction(selectedCount = getSelectedIds().length) {
        const buttonText = analyzeBtn.querySelector('.btn-text');
        if (selectedCount > 0) {
            buttonText.innerHTML = `<i class="fas fa-microchip"></i> Analisar selecionado${selectedCount > 1 ? 's' : ''}`;
            return;
        }
        buttonText.innerHTML = '<i class="fas fa-microchip"></i> Analisar documentos';
    }
    ...
    function displayResult(data, silent = false) {
    ...
    function analyzeDocuments() {
        if (mockDatabase.length === 0) {
            setStatus('Aguardando documentos', 'warning');
            showToast('Clique em Simular para carregar os documentos de teste.', 'warning');
            return;
        }

        const selectedIds = getSelectedIds();
        const documents = selectedIds.length > 0
            ? mockDatabase.filter(item => selectedIds.includes(item.id))
            : [...mockDatabase];

        setStatus('Analisando documentos...', 'warning');
        showToast(
            selectedIds.length > 0
                ? `Analisando ${documents.length} documento${documents.length > 1 ? 's selecionados' : ' selecionado'}.`
        ...
        setTimeout(() => {
            if (documents.length === 1) {
                displayResult(documents[0], true);
            } else {
                displayBatchResult(documents);
            }
            displayPanorama(documents, selectedIds.length > 0);
            setStatus('Análise concluída', 'success');
            analyzeBtn.disabled = false;
            analyzeBtn.classList.remove('loading');
            showToast('Análise concluída. O panorama geral foi atualizado.', 'success');
        }, 1500);
    }
    ...
    function getAnalysisSummary(documents) {
        const types = {};
        const involvedPeople = new Set();
        let totalValue = 0;

        documents.forEach(item => {
            const type = item.tipo_documento || 'Outro';
            types[type] = (types[type] || 0) + 1;
            totalValue += parseCurrency(item.campos_extraidos?.valor);
            (item.campos_extraidos?.envolvidos || []).forEach(person => involvedPeople.add(person));
        });
        ...
        return {
            ...
            types,
            totalValue,
            involved: involvedPeople.size,
            processedAt: new Date().toISOString()
        };
    }

    function displayPanorama(documents, selectedOnly) {
        const summary = getAnalysisSummary(documents);
        const typeBadges = Object.entries(summary.types)
            .sort((a, b) => b[1] - a[1])
            .map(([type, count]) => `<span class="panorama-type">${type}: <strong>${count}</strong></span>`)
            .join('');

        analysisPanorama.innerHTML = `
            <div class="panorama-header">
                <h4><i class="fas fa-chart-pie"></i> Panorama da análise</h4>
                <span>${selectedOnly ? 'Seleção analisada' : 'Lote completo analisado'}</span>
            </div>
        ...
        `;
        analysisPanorama.classList.add('show');
    }

    function displayBatchResult(documents) {
        const summary = getAnalysisSummary(documents);
        const mainType = Object.entries(summary.types).sort((a, b) => b[1] - a[1])[0]?.[0] || 'Documentos';
        const involved = [...new Set(documents.flatMap(item => item.campos_extraidos?.envolvidos || []))];

        resultTipo.textContent = 'Panorama geral';
        resultTipo.className = 'tag tag-outro';
        resultId.textContent = `${summary.total} documentos`;
        resultData.textContent = new Date().toLocaleDateString('pt-BR');
        resultLocal.textContent = 'Múltiplos locais';
        resultValor.textContent = formatCurrency(summary.totalValue);
        resultEnvolvidos.innerHTML = involved.slice(0, 6)
            .map(name => `<span class="envolvido-tag">${name}</span>`)
            .join('');
        resultResumo.textContent = `${summary.total} documentos analisados. O tipo mais frequente foi ${mainType}. Foram identificados ${Object.keys(summary.types).length} tipos de documento e ${summary.involved} pessoas distintas.`;
        resultProcessado.textContent = new Date(summary.processedAt).toLocaleString('pt-BR');
        resultArquivo.textContent = 'Lote de documentos';

        currentResult = {
            status: 'COMPLETED',
            panorama: summary,
            documentos: documents.map(item => item.id)
        };
        dynamoPreview.innerHTML = syntaxHighlight(JSON.stringify(currentResult, null, 2));
        resultsPanel.classList.add('show');
        emptyState.style.display = 'none';
    }

    function loadTestData() {
        setStatus('Carregando dados...', 'warning');
        ...
        setTimeout(() => {
            ...
            setStatus('Dados de teste carregados', 'success');
            showToast('Documentos de teste carregados para consulta.', 'success');
        }, 1000);
    }
    ...
    function clearAll() {
        ...
        analysisPanorama.classList.remove('show');
        analysisPanorama.innerHTML = '';
        ...
        setStatus('Área limpa', 'success');
        showToast('Todos os documentos foram removidos da tela.', 'info');
    }
    ...
    mockBtn.addEventListener('click', loadTestData);
    ...
    console.log('Sistema inicializado sem dados de teste');
